  ...
  categories.forEach((cat, index) => {
    categoryContainer.innerHTML += `
      <li data-category="${cat}"
          class="text-lg p-2 rounded-sm cursor-pointer ${
...
// Cards
const containersCards = (cards) => {
  cardsContainer.innerHTML = "";
  cards.forEach((card) => {
    cardsContainer.innerHTML += `
      <div class="card bg-white shadow-lg rounded-lg flex flex-col transition duration-300 hover:shadow-2xl hover:scale-105">
        <img src="${card.image}" alt="${card.name}" class="w-full h-48 object-cover rounded-t-lg"/>
        <div class="py-4 px-3 flex flex-col gap-2">
          <p class="plant-name font-bold text-lg cursor-pointer">${card.name}</p>
          <p class="text-sm text-justify">${card.description.slice(0, 80)}...</p>
          ...
          <button class="add-to-cart mt-2 text-sm bg-green-700 px-4 py-2 rounded-full text-white font-bold w-full hover:bg-yellow-700 transition-colors duration-300">
            Add to Cart
          </button>
        </div>
      </div>
...
const filterCardsByCategory = (categoryName) => {
  if (categoryName === "All Trees") {
    ...
    containersCards(allPlants.filter((p) => p.category === categoryName));
  }
};
...
cardsContainer.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("add-to-cart")) {
    ...
    const description = card.querySelector("p.text-sm.text-justify").innerText;
