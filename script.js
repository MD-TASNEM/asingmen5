      ...
      const loadCards = async () => {
        try {
          ...
          data.plants.forEach(plant => {
            if (!allCategories.includes(plant.category)) allCategories.push(plant.category);
          });
      ...
      const filterCardsByCategory = categoryName => {
        if (categoryName === "All Trees") renderCards(allPlants);
        else renderCards(allPlants.filter(p => p.category === categoryName));
      };

      // Category Click Event
      categoryContainer.addEventListener("click", e => {
        if (e.target.tagName === "LI") {
          categoryContainer.querySelectorAll("li").forEach(li => li.classList.remove("bg-green-700", "text-white"));
      ...
      cardsContainer.addEventListener("click", e => {
        ...
        if (e.target.classList.contains("add-to-cart")) {
          ...
          const price = parseInt(card.querySelector("span.font-bold.text-xl").innerText.replace("৳", ""));
